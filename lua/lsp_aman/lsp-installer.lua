...
for _, server in pairs(servers) do
  opts = {
    on_attach = require("lsp_aman.handlers").on_attach,
    capabilities = require("lsp_aman.handlers").capabilities,
  }

  if server == "sumneko_lua" then
    local sumneko_opts = require "lsp_aman.settings.sumneko_lua"
    opts = vim.tbl_deep_extend("force", sumneko_opts, opts)
  end

  if server == "pyright" then
    local pyright_opts = require "lsp_aman.settings.pyright"
